<div style="display: flex; flex-direction: column; gap: 1.5rem;">
    ...
    {{-- Header & KPIs --}}
    <div>
        <h2 style="font-size: 1.5rem; font-weight: 700; margin-bottom: 1rem; color: #111827;">Region: {{ $region }}</h2>
        <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 1rem;">
            <x-filament::section>
                <div style="color: #6b7280; font-size: 0.875rem;">Total Karyawan</div>
                <div style="font-size: 1.5rem; font-weight: 700;">{{ number_format($totalKaryawan) }}</div>
            </x-filament::section>
            <x-filament::section>
                <div style="color: #6b7280; font-size: 0.875rem;">Batch Aktif</div>
                <div style="font-size: 1.5rem; font-weight: 700; color: #185FA5;">{{ $activeBatch }}</div>
            </x-filament::section>
            <x-filament::section>
                <div style="color: #6b7280; font-size: 0.875rem;">Tingkat Kelulusan</div>
                <div style="font-size: 1.5rem; font-weight: 700; color: #0F6E56;">{{ $kelulusanPct }}%</div>
            </x-filament::section>
            <x-filament::section>
                <div style="color: #6b7280; font-size: 0.875rem;">RBH</div>
                <div style="font-size: 1.125rem; font-weight: 600; margin-top: 0.25rem;">{{ $rbhName }}</div>
            </x-filament::section>
        </div>
    </div>

    {{-- Area Breakdown Table --}}
    <x-filament::section heading="Breakdown per Area">
        <table style="width: 100%; border-collapse: collapse; font-size: 0.875rem;">
            ...
            <tbody>
                ...
                <tr onclick="window.location='{{ $areaRecord ? route('filament.admin.resources.areas.view', ['record' => $areaRecord->id]) : '#' }}'" 
                    ...
                    <td style="padding: 0.75rem; font-weight: 600;">{{ $row['area'] }}</td>
                    <td style="padding: 0.75rem;">{{ $row['abh'] }}</td>
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
        ...
        {{-- Recent Batches --}}
        <x-filament::section heading="Recent Batches">
